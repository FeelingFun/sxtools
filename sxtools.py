...
class SceneSetup(object):
    ...
    def createSXShader(self, numLayers, occlusion=False, specular=False,
        ...
        maya.cmds.relationship('link', ':lightLinker1', 'SXShaderSG.message',
                                                                                                    ':defaultLightSet.message')
        maya.cmds.relationship('shadowLink', ':lightLinker1', 'SXShaderSG.message',
                                                                                                    ':defaultLightSet.message')
    ...
    def createSXExportShader(self):
        ...
        maya.cmds.relationship('link', ':lightLinker1', 'SXExportShaderSG.message',
                                                                                                    ':defaultLightSet.message')
        maya.cmds.relationship('shadowLink', ':lightLinker1',
                                                                                                    'SXExportShaderSG.message',
                                                                                                    ':defaultLightSet.message')
    ...
    def createSXPBShader(self):
        ...
        maya.cmds.relationship('link', ':lightLinker1', 'SXPBShaderSG.message',
                                                                                                    ':defaultLightSet.message')
        maya.cmds.relationship('shadowLink', ':lightLinker1', 'SXPBShaderSG.message',
                                                                                                    ':defaultLightSet.message')
    ...
    def setPrimVars(self):
        ...
        for shape in settings.shapeArray:
            ...
            if attrList is None:
                maya.cmds.addAttr(
                    shape, ln='activeLayerSet', at='double', min=0, max=9, dv=0)
                maya.cmds.addAttr(
                    shape, ln='transparency', at='double', min=0, max=1, dv=0)
                maya.cmds.addAttr(shape, ln='shadingMode', at='double', min=0, max=2, dv=0)
                maya.cmds.addAttr(
                    shape, ln='occlusionVisibility', at='double', min=0, max=1, dv=1)
                maya.cmds.addAttr(
                    shape, ln='specularVisibility', at='double', min=0, max=1, dv=1)
                maya.cmds.addAttr(
                    shape, ln='transmissionVisibility', at='double', min=0, max=1, dv=1)
                maya.cmds.addAttr(
                    shape, ln='emissionVisibility', at='double', min=0, max=1, dv=1)
                maya.cmds.addAttr(
                    shape, ln='occlusionBlendMode', at='double', min=0, max=2, dv=0)
                maya.cmds.addAttr(
                    shape, ln='specularBlendMode', at='double', min=0, max=2, dv=0)
                maya.cmds.addAttr(
                    shape, ln='transmissionBlendMode', at='double', min=0, max=2, dv=0)
                maya.cmds.addAttr(
                    shape, ln='emissionBlendMode', at='double', min=0, max=2, dv=0)
                ...
                if ('activeLayerSet' not in attrList):
                    maya.cmds.addAttr(
                        shape, ln='activeLayerSet', at='double', min=0, max=10, dv=0)
                if ('transparency' not in attrList):
                    maya.cmds.addAttr(
                        shape, ln='transparency', at='double', min=0, max=1, dv=0)
                if ('shadingMode' not in attrList):
                    maya.cmds.addAttr(
                        shape, ln='shadingMode', at='double', min=0, max=2, dv=0)
...
class ToolActions(object):
    ...
    def assignToCreaseSet(self, setName):
        creaseSets = ('sxCrease0', 'sxCrease1', 'sxCrease2', 'sxCrease3',
                                                                'sxCrease4')
        for component in settings.componentArray:
            if ((maya.cmds.filterExpand(component, sm=31) is not None) or
                            (maya.cmds.filterExpand(component, sm=32) is not None)):
    ...
    def applyTexture(self, texture, uvSetName, applyAlpha):
        ...
        for shape in settings.shapeArray:
            ...
            components = maya.cmds.ls(
                maya.cmds.polyListComponentConversion(shape, tv=True), fl=True)
    ...
    def gradientFill(self, axis):
        ...
        for component in components:
            ...
            if axis == 1:
                ...
                ratioRaw = (compPos[2] - objectBoundsZmin) / (
    ...
    def swapLayers(self, object):
        ...
        if (layerA in refLayers) and (layerB in refLayers):
            ...
            print('SXTools Error: Invalid layers on ' + str(object))
    ...
    def getLayerMask(self):
        ...
        for shape in settings.shapeArray:
            vertFaceList = maya.cmds.ls(
                maya.cmds.polyListComponentConversion(shape, tvf=True), fl=True)
    ...
    def copyFaceVertexColors(self, object, layerA, layerB):
        selectionList = OM.MSelectionList()
        selectionList.add(object)
        nodeDagPath = OM.MDagPath()
        nodeDagPath = selectionList.getDagPath(0)
        ...
        maya.cmds.polyColorSet(object, currentColorSet=True, colorSet=layerB)
        MFnMesh.setFaceVertexColors(layerAColors, faceIds, vtxIds)
...
class LayerManagement(object):
        ...
        for object in objects:
            ...
            if var == 0:
                for layer in refLayers:
                    ...
                    maya.cmds.polyColorSet(object, copy=True,
                    colorSet=layer, newColorSet=layerName)
                    tools.copyFaceVertexColors(object, layer, layerName)
                var += 1
                for layer in refLayers:
                    layerName = str(layer) + '_var' + str(var)
                    maya.cmds.polyColorSet(object, copy=True,
            ...
            print('SX Tools: Layer Set ' + str(var) + ' added to ' + str(object))
            maya.cmds.polyColorSet(object, currentColorSet=True, colorSet='layer1')
    ...
    def setColorSet(self, highlightedLayer):
        for shape in settings.shapeArray:
            maya.cmds.polyColorSet(
                shape, currentColorSet=True, colorSet=highlightedLayer)
...
class UI(object):
    ...
    def bakeOcclusionToolUI(self):
        ...
        maya.cmds.rowColumnLayout(
            ...
            columnAttach=[(1, 'left', 0), (2, 'left', 0), (3, 'left', 0), (4, 'left',
                                                                                                                                                                                                                                                                        0)],
            rowSpacing=(1, 0))
    ...
    def swapLayerToolUI(self):
        ...
        maya.cmds.button(
            ...
            height=30,
            width=100,
            command=('sxtools.tools.swapLayers(sxtools.settings.shapeArray[len(sxtools.settings.shapeArray)-1])'))
        maya.cmds.setParent('toolFrame')
